...
try:
    ...
    # Apply SQL queries on data
    query1 = 'SELECT * FROM input_data WHERE age >= 18'
    query2 = 'SELECT name, address FROM query1_data WHERE address LIKE "%Street%"'
    query3 = 'SELECT name, address, age FROM query2_data WHERE age <= 30'

    query1_data = input_df.filter(query1)
    query2_data = query1_data.filter(query2)
    query3_data = query2_data.filter(query3)

    # Create DynamicFrame from processed data
    output_dyf = DynamicFrame.fromDF(query3_data, glueContext, 'output_dyf')
    ...
    # Handle exceptions
    error_message = f'Error processing data
